...
namespace HTMLConverter {
	...
	public static class HtmlToXamlConverter {
			...
			else if (htmlNode is XmlElement)
			{
				...
				switch (htmlElementName)
				{
					...
					case "img":
						...
						AddImage(xamlParentElement, htmlElement, inheritedProperties, stylesheet, sourceContext, true);
						break;
		...
		private static void AddImage(XmlElement xamlParentElement, XmlElement htmlElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext, bool isParagraph = false) {
			Hashtable localProperties;
			Hashtable currentProperties = GetElementProperties(htmlElement, inheritedProperties, out localProperties, stylesheet, sourceContext);

			XmlElement paragraphElement = xamlParentElement.OwnerDocument.CreateElement(null, Xaml_Paragraph, _xamlNamespace);
			XmlElement uiContainerXamlElement = (isParagraph ? paragraphElement : xamlParentElement).OwnerDocument.CreateElement(null, "InlineUIContainer", _xamlNamespace);
			XmlElement imageElement = uiContainerXamlElement.OwnerDocument.CreateElement(null, "Image", _xamlNamespace);
			imageElement.SetAttribute("Source", htmlElement.GetAttribute("src"));
			imageElement.SetAttribute("Stretch", "Uniform");
			imageElement.SetAttribute("StretchDirection", "DownOnly");

			ApplyLocalProperties(imageElement, localProperties, true);

			uiContainerXamlElement.AppendChild(imageElement);
			paragraphElement.AppendChild(uiContainerXamlElement);
			xamlParentElement.AppendChild(isParagraph ? paragraphElement : uiContainerXamlElement);
		}
